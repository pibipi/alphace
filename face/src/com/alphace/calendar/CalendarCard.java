...
public class CalendarCard extends View {
	...
	//
	private ArrayList<CustomDate> multiply_test_date_list;
	...
	private void init(Context context) {
		//
		multiply_test_date_list = NomalConstant.multiply_test_date_list;

		mTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
		mCirclePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
		mCirclePaint.setStyle(Paint.Style.FILL);
		mCirclePaint.setColor(Color.parseColor("#901d20")); // 红色圆形
	...
	private void fillDate() {
		// System.out.println(mShowDate.year + "``" + mShowDate.month + "``" +
		// mShowDate.day + "``" + "ymd");
		...
		int firstDayWeek = DateUtil.getWeekDayFromDate(mShowDate.year, mShowDate.month);// 第一周第一天在当前月的位置

		boolean isCurrentMonth = false;
		if (DateUtil.isCurrentMonth(mShowDate)) {
			isCurrentMonth = true;
		}
		boolean isUnReachMonth = false;
		if (DateUtil.isUnReachMonth(mShowDate)) {
			isUnReachMonth = true;
		}
		int day = 0;
		for (int j = 0; j < TOTAL_ROW; j++) {
			...
			for (int i = 0; i < TOTAL_COL; i++) {
				...
				if (position >= firstDayWeek && position < firstDayWeek + currentMonthDays) {
					...
					if ((isCurrentMonth && day > monthDay) || isUnReachMonth) { // 如果比这个月的今天要大，表示还没到
				...
				// TODo
				if (isMultiplyTestDay(mShowDate.year, mShowDate.month, position - firstDayWeek + 1)) {
					rows[j].cells[i] = new Cell(
							(new CustomDate(mShowDate.year, mShowDate.month, position - firstDayWeek + 1)),
							State.Multiply_Test_Day, i, j);
				}
				if (isCurrentMonth && day == monthDay
						&& isMultiplyTestDay(mShowDate.year, mShowDate.month, position - firstDayWeek + 1)) {
					rows[j].cells[i] = new Cell(
							(new CustomDate(mShowDate.year, mShowDate.month, position - firstDayWeek + 1)),
							State.Multiply_Test_And_Today, i, j);
				}
				// if (mShowDate.year == 2015 && mShowDate.month == 8 &&
				// position == (8 + firstDayWeek - 1)) {
	...
	private boolean isMultiplyTestDay(int year, int month, int day) {
		for (CustomDate date : multiply_test_date_list) {
	...
	class Cell {
		...
		public void drawSelf(Canvas canvas) {
			...
			case TODAY: // 今天
				...
				canvas.drawCircle((float) (mCellSpace * (i + 0.5)), (float) ((j + 0.5) * mCellSpace), mCellSpace / 3.5f,
						mCirclePaint);
				break;
			...
			case UNREACH_DAY: // 还未到的天
				...
				break;
			case Multiply_Test_Day:
				// TODo
				// mTextPaint.setColor(Color.parseColor("#60B8FD"));

				mCirclePaint.setColor(Color.parseColor("#00d1c4"));
				canvas.drawCircle((float) (mCellSpace * (i + 0.5)), (float) ((j + 1.0f) * mCellSpace), mCellSpace / 15,
						mCirclePaint);
				break;
			case Multiply_Test_And_Today:
				mCirclePaint.setColor(Color.parseColor("#901d20")); // 红色圆形
				mTextPaint.setColor(Color.parseColor("#fffffe"));
				canvas.drawCircle((float) (mCellSpace * (i + 0.5)), (float) ((j + 0.5) * mCellSpace), mCellSpace / 3.5f,
						mCirclePaint);
	 ...
	 */
	enum State {
		TODAY, CURRENT_MONTH_DAY, PAST_MONTH_DAY, NEXT_MONTH_DAY, UNREACH_DAY, Multiply_Test_Day, Multiply_Test_And_Today;
	}
