...
public class RoundProgressBar extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		// canvas.drawCircle(centre, centre, radius+roundWidth*0.45f, paint);
		...
		paint.setTypeface(type_face); // 设置字体
		...
		//
		Paint mp = new Paint();
		mp.setColor(roundProgressColor); // 设置进度的颜色
		mp.setStrokeWidth(0);
		mp.setAntiAlias(true); // 消除锯齿
		canvas.drawCircle(centre, centre - radius, roundWidth * 0.5f, mp);
		float x1 = (float) ((float) centre + (float) radius
				* Math.sin((double) ((float) progress / 100f * 2 * Math.PI)));
		float y1 = (float) ((float) centre - (float) radius
				* Math.cos((double) ((float) progress / 100f * 2 * Math.PI)));
		canvas.drawCircle(x1, y1, roundWidth * 0.5f, mp);
		//
		/**
		 * 画圆弧 ，画圆环的进度
		 */
